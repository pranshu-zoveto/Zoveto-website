    ...
    >
      ...
      <h3
        className={cn(
          ...
          dense ? "text-lg leading-snug sm:text-[1.06rem]" : "text-[1.06rem] leading-snug md:text-[1.15rem]",
        )}
      >
        {proof.title}
      </h3>
        ...
        <div>
          ...
          <ul className="space-y-1.5 text-sm text-muted">
            {proof.after.map((item) => (
              <li key={item} className="flex items-start gap-2">
                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-blue" aria-hidden />
                <span>{item}</span>
              </li>
            ))}
          </ul>
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-sm font-semibold text-foreground">
              {proof.outcomeMetrics.map((item) => (
                <li key={item} className="flex items-start gap-2">
                  <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-blue/90" aria-hidden />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
